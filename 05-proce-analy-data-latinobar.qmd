...
l2023 <- read_dta("input/data/latinobar/2023.dta", encoding = "UTF-8")
```
...
porcentaje_confianza <- resultado_bruto %>%
  ...
  filter(!pais %in% c("España", "Rep. Dominicana"))  # Filtrar países no deseados
```
...
ggplot(porcentaje_confianza, aes(x = anio, y = porcentaje)) +
  geom_line() +
  geom_point() +
  facet_wrap(~ pais, ncol = 3) +  # 3 columnas, filas automáticas
...
```{r}
ggsave("output/graph_trust_political_parties.png", width = 10, height = 15)
```

```{r}
ggplot(porcentaje_confianza, aes(x = anio, y = porcentaje, color = pais)) +
  geom_line() +
  geom_point() +
  labs(title = "Trust in political parties by country",
